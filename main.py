...
class Management:
    ...
    def AddStudent(self):
        ...
        self.stu_id= Label(self.frame_1, text="Student Id", font=(self.font_2, 15, "bold"), bg=self.color_1).place(x=40,
                                                                                                                  y=310)
        self.stu_id_entry = Entry(self.frame_1, bg=self.color_4, fg=self.color_3)
        self.stu_id_entry.place(x=40, y=340, width=200)
    ...
    def GetContact_View(self):
        ...
        self.getInfo = Label(self.frame_1, text="Enter Student Id", font=(self.font_2, 18, "bold"),
    ...
    def CheckContact_View(self):
        if self.getInfo_entry.get() == "":
            messagebox.showerror("Error!", "Please enter your id", parent=self.window)
        else:
            try:
                connection = pymysql.connect(host=self.host, user=self.user, password=self.password,
                                             database=self.database)
                curs = connection.cursor()
                curs.execute("select * from student_register where stu_id=%s", self.getInfo_entry.get())
                row = curs.fetchone()
        ...
        if self.getInfo_entry.get() == "":
            ...
            try:
                ...
                if row == None:
                    ...
                    curs.execute("delete from student_register where stu_id=%s", self.getInfo_entry.get())
                    connection.commit()
    ...
    def GetUpdateDetails(self, row):
        ...
        stu_id = Label(self.frame_1, text="Student Id", font=(self.font_2, 15, "bold"), bg=self.color_1).place(x=40,
                                                                                                             y=310)
        stu_id_data = Label(self.frame_1, text=row[8], font=(self.font_1, 10)).place(x=40, y=340)
        ...
        if self.name_entry.get() == "" or self.surname_entry.get() == "" or self.course_entry.get() == "" or self.subject_entry.get() == "" or self.year_entry.get() == "" or self.age_entry.get() == "" or self.gender_entry.get() == "" or self.birth_entry.get() == "" or self.email_entry.get() == "":
            ...
            try:
                connection = pymysql.connect(host=self.host, user=self.user, password=self.password,
                                             database=self.database)
                curs = connection.cursor()
                curs.execute("select * from student_register where stu_id=%s", row[8])
                row = curs.fetchone()
                ...
                if row == None:
                    ...
                    curs.execute(
                        "update student_register set f_name=%s,l_name=%s, course=%s, subject=%s, year=%s, age=%s, gender=%s, birth=%s, email=%s where stu_id=%s",
                        (
                            self.name_entry.get(),
    ...
    def Submit(self):
        if self.name_entry.get() == "" or self.surname_entry.get() == "" or self.course_entry.get() == "" or self.subject_entry.get() == "" or self.year_entry.get() == "" or self.age_entry.get() == "" or self.gender_entry.get() == "" or self.birth_entry.get() == "" or self.stu_id_entry.get() == "" or self.email_entry.get() == "":
            messagebox.showerror("Error!", "Sorry!, All fields are required", parent=self.window)
        else:
            try:
                connection = pymysql.connect(host=self.host, user=self.user, password=self.password,
                                             database=self.database)
                curs = connection.cursor()
                curs.execute("select * from student_register where stu_id=%s", self.stu_id_entry.get())
                row = curs.fetchone()

                if row != None:
                    messagebox.showerror("Error!",
                                         "The student id is already exists, please try again with another id",
                                         parent=self.window)
                else:
                    curs.execute(
                        "insert into student_register (f_name,l_name,course,subject,year,age,gender,birth,stu_id,email) values(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)",
                        (
                            self.name_entry.get(),
                            self.surname_entry.get(),
                            self.course_entry.get(),
